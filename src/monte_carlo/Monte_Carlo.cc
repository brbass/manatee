    ...
    check_weight_windows(Particle &particle)
    {
        ...
        else if (particle.weight() > weight_windows(particle.cell(), particle.group()).upper) // assumes ideal <= 2 * upper
        {
            ...
            if (particles_to_create == 0)
            {
                ...
                double lower = weight_windows(particle.cell(), particle.group()).lower;
                double upper = weight_windows(particle.cell(), particle.group()).ideal;
                double ideal = weight_windows(particle.cell(), particle.group()).upper;

                unsigned g = particle.group();
                unsigned i = particle.cell();
            }
    ...
    initialize_weight_windows(vector<double> &phi_adjoint)
    {
        ...
        for (unsigned i = 0; i < mesh_.number_of_cells(); ++i)
        {
            for (unsigned g = 0; g < data_.number_of_groups(); ++g)
            {
                ...
                weight_windows(i, g).lower = response / phi[k];
