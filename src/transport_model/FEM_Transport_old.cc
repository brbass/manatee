...
namespace transport_ns
{
    ...
    spherical_sweep(vector<double> &psi,
                    vector<double> &q)
    {
        vector<double> psi_boundary_sources(data_.number_of_groups() * number_of_angular_nodes_, 0);
        ...
        if (data_.boundary_condition(1).compare("reflected") == 0)
        {
            for (unsigned g = 0; g < data_.number_of_groups(); ++g)
            {
                ...
                unsigned k2 = 1 + mesh_.number_of_nodes() * (g + data_.number_of_groups() * ((number_of_angular_nodes_ - 1 - o) + number_of_angular_nodes_ * (mesh_.number_of_cells() - 1)));
        ...
        sweep_inward(psi, psi_boundary_sources, q);
        
        // boundary condition, r=0
        if (data_.boundary_condition(0).compare("reflected") == 0)
        {
            for (unsigned o = number_of_angular_nodes_ / 2; o < number_of_angular_nodes_; ++o)
            {
                for (unsigned g = 0; g < data_.number_of_groups(); ++g)
                {
                    unsigned k1 = g + data_.number_of_groups() * o;
                    unsigned k2 = 0 + mesh_.number_of_nodes() * (g + data_.number_of_groups() * ((number_of_angular_nodes_ - 1 - o) + number_of_angular_nodes_ * 0));
        ...
        sweep_outward(psi, psi_boundary_sources, q);

        // print_angular_flux(psi);
    }

    void FEM_Transport::
    sweep_inward(vector<double> &psi,
                 vector<double> &psi_boundary_sources,
                 vector<double> &q)
    {
        for (unsigned o = 0; o < number_of_angular_nodes_ / 2; ++o)
        {
            ...
            { 
                for (unsigned g = 0; g < data_.number_of_groups(); ++g)
                {
                    ...
                    {
                        unsigned k = mesh_.number_of_nodes() * (g + data_.number_of_groups() * (o + number_of_angular_nodes_ * (i + 1)));
                    ...
                    for (unsigned n = 0; n < mesh_.number_of_nodes(); ++n)
                    {
                        unsigned k = n + mesh_.number_of_nodes() * (g + data_.number_of_groups() * (o + number_of_angular_nodes_ * i));
    ...
    void FEM_Transport::
    sweep_outward(vector<double> &psi,
                  vector<double> &psi_boundary_sources,
                  vector<double> &q)
    {
        for (unsigned o = number_of_angular_nodes_ / 2; o < number_of_angular_nodes_; ++o)
        {
            ...
            { 
                for (unsigned g = 0; g < data_.number_of_groups(); ++g)
                {
                    ...
                    if (i > 0)
                    {
                        unsigned k = 1 + mesh_.number_of_nodes() * (g + data_.number_of_groups() * (o + number_of_angular_nodes_ * (i - 1)));
        ...
        for (unsigned i = 0; i < mesh_.number_of_cells(); ++i)
        {
            for (unsigned g = 0; g < data_.number_of_groups(); ++g)
            {
                for (unsigned n = 0; n < mesh_.number_of_nodes(); ++n)
                {
                    ...
                    {
                        unsigned k1 = n + mesh_.number_of_nodes() * (g + data_.number_of_groups() * (o + number_of_angular_nodes_ * i));
    ...
    solve()
    {
        ...
        // temporary variables
        vector<double> psi(mesh_.number_of_cells() * data_.number_of_groups() * mesh_.number_of_nodes() * number_of_angular_nodes_, 0);
                      ...
                      vector<double> &leakage)
    {
        ...
        for (unsigned g = 0; g < data_.number_of_groups(); ++g)
        {
            unsigned kl1 = 0 + mesh_.number_of_nodes() * (g + data_.number_of_groups() * (0 + number_of_angular_nodes_ * il));
            unsigned kl2 = 0 + mesh_.number_of_nodes() * (g + data_.number_of_groups() * (1 + number_of_angular_nodes_ * il));
            unsigned kr1 = 1 + mesh_.number_of_nodes() * (g + data_.number_of_groups() * (0 + number_of_angular_nodes_ * ir));
            unsigned kr2 = 1 + mesh_.number_of_nodes() * (g + data_.number_of_groups() * (1 + number_of_angular_nodes_ * ir));
            
            leakage[0] += psi[kl1] * 3.0 / 4.0  + psi[kl2] / 4.0;
            leakage[1] += psi[kr1] / 4.0 + psi[kr2] * 3.0 / 4.0;
        }
    }
}
